...
// Supabase credentials - get from your .env or hardcode temporarily
...
async function seedMeals() {
  ...
  try {
    ...
    if (existingMeals && existingMeals.length > 0) {
      ...
      console.log('Do you want to continue? This will add duplicate meals.')
      console.log('If you want to clear existing meals first, run:')
      console.log('DELETE FROM meals;')
      console.log('\nProceeding anyway in 3 seconds...')
    ...
    for (let i = 0; i < meals.length; i += batchSize) {
      ...
      if (error) {
        ...
        console.log(`✅ Batch ${i / batchSize + 1} inserted successfully (${successCount}/${meals.length})`)
      }
    }
    ...
    if (!verifyError && allMeals) {
      const counts = allMeals.reduce((acc: any, meal: any) => {
